...
public class KafkaMain {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		EmployeeDeserializer kk=new EmployeeDeserializer();
		ConvertJson bk=new ConvertJson();
		
		KStream<String, String> left = builder.stream("topic2");
		//KStream<String, Employee> Eleft=kk.deserialize("topic2", left);
		left.foreach((key,value)->System.err.println(key+"--->"+value));
		
//		left.mapValues(value -> value.replace("}", ""));
//		left.foreach((key, value) -> System.out.println(key + " =left> " + value));
//	    KStream<String, String> right = builder.stream("topic3");
//	    KStream<String, String> newright =left.mapValues(value -> value.replace("{", ""));
//	    newright.foreach((key, value) -> System.out.println(key + " =right> " + value));
